...
import { Check, ChevronDown, Plus, Tag as AngleIcon, X } from "lucide-react";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
/**
 * Multi-select angle dropdown: pick from the existing angle library (checkbox
 * list) and/or type a brand-new angle and press Enter / "Add". Selected angles
 * render as removable chips beneath the trigger. Presentational — the parent
 * owns persistence via `onChange`.
 */
export function AngleMultiSelect({
  value,
  onChange,
  allAngles,
  disabled,
  ...
  // The full option list: library angles ∪ whatever is currently selected
  // (so a freshly-added angle still shows as a checked row), sorted, filtered
  // by the typed query.
  const options = useMemo(() => {
    const all = Array.from(new Set([...allAngles, ...value])).sort((a, b) =>
      a.localeCompare(b),
    );
    const q = draft.trim().toLowerCase();
    return q ? all.filter((t) => t.toLowerCase().includes(q)) : all;
  }, [allAngles, value, draft]);

  const toggle = (angle: string) => {
    if (selected.has(angle)) onChange(value.filter((t) => t !== angle));
    else onChange([...value, angle]);
  };
        ...
        <PopoverTrigger asChild>
          ...
          >
            ...
            <span>
              {value.length === 0
                ? "Add angles"
                : `${value.length} angle${value.length === 1 ? "" : "s"} selected`}
            </span>
          ...
          <div className="p-2 border-b border-line">
            <div className="flex items-center gap-1.5">
              <input
                ...
                placeholder="Search or add an angle…"
            ...
            {options.length === 0 ? (
              <div className="px-2 py-3 text-center text-[11px] text-ink-3">
                {draft.trim()
                  ...
                  : "No angles yet. Type one above."}
              </div>
            ) : (
              options.map((angle) => {
                const isSel = selected.has(angle);
                return (
                  <button
                    key={angle}
                    type="button"
                  ...
                  >
                    ...
                    <span className="flex-1 text-left truncate">{angle}</span>
                  </button>
                );
              })
            )}
