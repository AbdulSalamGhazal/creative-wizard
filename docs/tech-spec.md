...
**Budget (§Budget module):** `budget_allocations` (monthly spend plan per platform × objective, USD; `month` = first-of-month; unique `(account_id, month, platform, objective)`), `budget_targets` (one row per month: `planned_revenue_sar`; unique `(account_id, month)` — month-grain so future targets slot in). Both tenant tables.
...
## 5d. Budget module (monthly plan vs actual)

- **What:** `/budget` (Ads section) — month-grain plan vs actual. Spend planned in **USD** per platform → objective (both pickers derive from `lib/palette` / `CAMPAIGN_OBJECTIVES`); revenue planned as ONE monthly **SAR** total per brand (no breakdown — deliberate). Any month editable (`budget.manage`, editor preset; audited `budget.update`) — no locking.
...
- **Actuals — STANDING DECISIONS (do not "fix"):** actual spend = the month's `performance_records` **with NO exclusion filtering** (raw totals; budget answers "what did we actually spend", so its totals may differ slightly from dashboard aggregates, which do filter). Actual revenue = `SUM(store_orders.total_amount)` for the month (store FACTS; claimed-vs-real analysis stays on Reconciliation) + order count as context.
...
- **Currency:** revenue always SAR. Spend is USD natively with a USD↔SAR display toggle (per-user, localStorage `cw-budget-currency`) converting via the per-brand `accounts.usd_to_sar_rate` (default **3.77**, inline-editable with `budget.manage`, validated 0 < r ≤ 100). Target/actual **ROAS is always computed through the rate** (revenue SAR ÷ spend×rate), regardless of the toggle.
- **Pacing (current month only):** expected-to-date = plan × elapsed ÷ days-in-month; deviations are warn-tinted by |magnitude| (over- and under-pace are both deviations — Reconciliation's Δ philosophy). Past months show plain plan-vs-actual. Pure math in `lib/budget.ts` (unit-tested).
- Edit mode = house draft + dirty-check + explicit Save (full-replace per month); "Copy from last month" (replaces, confirm when the month already has a plan); CSV export in the displayed currency. Migration **0035** (additive: the two tables + the rate column).
...
- **Ads:** `/` (dashboard) · `/summary` · `/creatives` (+ `/new`, `/bulk`, `/[name]`) · `/campaigns` (+ `/new`, `/[campaign]`) · `/budget` · `/funnel` · `/compare` · `/trends/{over-time,by-type,by-tag,launches,video}` · `/uploads` (+ `/new`).
